...
void lcd_clock_on(void)
{
	if (g_kona_fb)
		if (!g_kona_fb->display_info->vmode)
			g_kona_fb->display_ops->start(g_kona_fb->display_hdl, &g_kona_fb->dfs_node);
}

void lcd_clock_off(void)
{
	if (g_kona_fb)
		if (!g_kona_fb->display_info->vmode)
			g_kona_fb->display_ops->stop(g_kona_fb->display_hdl, &g_kona_fb->dfs_node);
}
...
#else
	sec_getlog_supply_fbinfo((void*)((vmalloc_to_pfn(fb->fb.screen_base)<<PAGE_SHIFT)-PHYS_OFFSET),
...
#ifdef CONFIG_PRERESERVE_BOOTLOADER_FB
		uboot_kvirt = phys_to_virt(uboot_phys);
...
int kona_fb_obtain_lock(void)
{
	...
	if (!fb->display_info->vmode)
		wait_for_completion_interruptible_timeout(
			&fb->prev_buf_done_sem,	msecs_to_jiffies(10));
	kona_clock_start(fb);
