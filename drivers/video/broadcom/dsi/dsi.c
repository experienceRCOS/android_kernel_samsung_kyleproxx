...
#ifdef CONFIG_MACH_HAWAII_SS_HEAT3G
...
int panel_read_HSmode(UInt8 reg, UInt8 *rxBuff, UInt8 buffLen)
{
	int ret = 0;
	DispDrv_PANEL_t * pPanel = panel;
	bool original_cmnd_LP;

	original_cmnd_LP = pPanel->disp_info->cmnd_LP;
	pPanel->disp_info->cmnd_LP = false;
	ret = DSI_DCS_Read(panel, reg, rxBuff, buffLen);
	pPanel->disp_info->cmnd_LP = original_cmnd_LP;
	return ret;
}
EXPORT_SYMBOL(panel_read_HSmode);
/*
 *
