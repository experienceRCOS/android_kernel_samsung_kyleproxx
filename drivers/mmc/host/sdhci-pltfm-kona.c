...
kona_sdio_regulator_power(struct sdio_dev *dev, int power_state)
{
	...
	if (dev->devtype == SDIO_DEV_TYPE_SDMMC && dev->vddo_sd_regulator &&
		...
		if (power_state && !dev->sdio_regulator_enable) {
			...
			/***
				NANI
				To check regulator control
			***/
			printk( "[NANI] %s, %d : Turning ON sdldo and sdxldo, %s[%s], configure_sdio_pullup(%#x)\n",
				__FUNCTION__, __LINE__, mmc_hostname(dev->host->mmc), dev_name(mmc_dev(dev->host->mmc)), hw_cfg->configure_sdio_pullup );
#endif
			...
			/***
				...
				To check regulator control
			***/
			printk( "[NANI] %s, %d : Turning OFF sdldo and sdxldo, %s[%s], configure_sdio_pullup(%#x)\n",
				__FUNCTION__, __LINE__, mmc_hostname(dev->host->mmc), dev_name(mmc_dev(dev->host->mmc)), hw_cfg->configure_sdio_pullup );
#endif
